...
const singleTier = `<tr>
                        <td class='labelHolder' contenteditable='true' style='background-color:#FFFF7F'> 
                            <span class='label'>NEW</span>
                        </td> 
                        <td class='tierHolder'>
                            "<div class='tier sort' id='rows'>
                                <div class='hiddenkid'></div>
                            </div>
                        </td>
                        <td class='settingsContainer'>
                            <div id='settings'></div>
                            <div id='moveButtons'>
                                <div id='moveUp'></div>
                                <div id='moveDown'></div>
...
function changeCharacterCheckbox(selected) {
    ...
    if ($(selected).is(":checked")) {
        for (var index = 0; index <= currentGameChars[gameIndex][1]; index++) {
            var image = new Image();
            image.src = "img/" + currentGame + "/" + gameId + "/" + index + ".png";
            if (image.width == 0) {
                //This if-else statement exists to prevent empty images from displaying
            }
            else {
                $("#char div:last-child").after(singleChar);
                $("#char div:last-child").css("background-image", `url(${image.src})`);
                $("#char div:last-child").attr("id", gameId + "-" + index);
            }
        }
    } else {
